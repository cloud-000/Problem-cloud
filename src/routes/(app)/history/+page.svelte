<script lang="ts">
    ...
    let stats = $derived.by(() => {
        ...
        const accuracy = graded.length === 0 
            ? "0%" 
        ...
        // Time spent
        const totalMs = filteredSubmissions.reduce((acc, s) => acc + (s.elapsed_ms ?? 0), 0);
...
<div class="flex flex-col gap-6 p-6 max-w-5xl mx-auto w-full">
    ...
    <div class="border-b border-border/80 pb-4 space-y-1">
        <h1 class="text-3xl font-semibold tracking-tight text-foreground flex items-center gap-2">
            <Icon name="history" fontsize="2rem" class="text-primary-foreground" />
            History
        </h1>
        <p class="text-sm text-muted-foreground">
            Track your practice history and review past problem-solving attempts.
        </p>
    </div>
    ...
    {#if !user}
        ...
        <div class="flex flex-col items-center justify-center gap-4 text-center py-16">
            <div class="flex size-16 items-center justify-center rounded-full bg-surface-container text-muted-foreground">
                <Icon name="history" fontsize="2.5rem" />
            </div>
            <div class="flex max-w-sm flex-col gap-1">
                <h2 class="text-lg font-semibold">Sign in to view your history</h2>
                <p class="text-sm text-muted-foreground">
                    We track your progress and submission history automatically once you are logged in.
                </p>
            </div>
            <Button href="/auth/login" class="bg-primary text-primary-foreground hover:bg-primary/95 mt-2 px-6 shadow-sm">
                Log In
            </Button>
        </div>
    {:else}
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="p-4 rounded-xl border border-border/60 bg-surface-container-low flex flex-col justify-center">
                <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Total Attempts</div>
                <div class="mt-2 text-2xl font-bold text-foreground font-mono">{stats.total}</div>
            </div>
            <div class="p-4 rounded-xl border border-border/60 bg-surface-container-low flex flex-col justify-center">
                <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Accuracy</div>
                <div class="mt-2 text-2xl font-bold text-foreground font-mono">{stats.accuracy}</div>
            </div>
            <div class="p-4 rounded-xl border border-border/60 bg-surface-container-low flex flex-col justify-center">
                <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Solved Count</div>
                <div class="mt-2 text-2xl font-bold text-foreground font-mono">{stats.solved}</div>
            </div>
            <div class="p-4 rounded-xl border border-border/60 bg-surface-container-low flex flex-col justify-center">
                <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Time Spent</div>
                <div class="mt-2 text-2xl font-bold text-foreground font-mono">{stats.timeSpent}</div>
            </div>
        </div>

        <!-- Filters Panel -->
        <div class="flex flex-col md:flex-row gap-4 items-end bg-surface-container-low p-4 rounded-xl border border-border/60">
            <div class="flex flex-col gap-1.5 md:w-64 w-full">
                <span class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Outcome</span>
                <Combobox
            ...
            <div class="flex flex-col gap-1.5 md:w-48 w-full">
                <span class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Time Range</span>
                <Select
            ...
            <div class="flex flex-col items-center justify-center py-16 gap-3 text-center">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted-foreground mt-0.5">
                        {hasActiveFilters ? "Try resetting the filters." : "Start practicing to see your attempts here!"}
                    </p>
                </div>
                {#if hasActiveFilters}
                    <Button size="sm" onclick={resetFilters} class="mt-1">Clear Filters</Button>
                {:else}
        ...
        {:else}
            <div class="space-y-6">
                {#each groupedSubmissions as group}
                    <div class="space-y-3">
                        <h2 class="text-xs font-bold uppercase tracking-wider text-muted-foreground border-b border-border/40 pb-1">
                            {group.dayLabel}
                        </h2>
                        ...
                        <div class="space-y-2">
                            {#each group.items as sub (sub.id)}
                                ...
                                <div class="rounded-xl border border-border/60 bg-surface-container-lowest overflow-hidden transition-all duration-200 shadow-xs hover:border-border">
                                    ...
                                    >
                                        <div class="flex items-center gap-3 min-w-0 flex-1">
                                            ...
                                            <div class="flex flex-col min-w-0">
                                                ...
                                                <span class="text-xs text-muted-foreground flex items-center gap-1.5 mt-0.5">
                                                    <span>{formatTimeOfDay(sub.created_at)}</span>
                                                    <span>•</span>
                                                    <span>{formatElapsed(sub.elapsed_ms)}</span>
                                                    {#if sub.source}
                                                        <span>•</span>
                                                        <span class="uppercase tracking-wider text-[9px] font-bold bg-muted px-1 py-0.2 rounded border border-border/50">
                                                            {sub.source}
                                                        </span>
                                                    {/if}
                                                </span>
                                        ...
                                        <div class="flex items-center gap-3 shrink-0">
                                            {#if sub.flagged}
                                                <Icon name="flag" class="text-unsure size-[1.2rem]" fill />
                                            {/if}
                                            <Icon
                                    ...
                                    {#if isExpanded}
                                        <div class="border-t border-border/40 p-4 bg-surface-container-low/10">
                                            {#if sub.problems}
                                                <Problem
                                                ...
                                                />
                                            {:else}
                                                <p class="text-xs text-muted-foreground italic">Problem details could not be loaded.</p>
                                            {/if}
                                        </div>
                                    {/if}
                                </div>
