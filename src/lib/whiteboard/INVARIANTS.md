...
| Smart non-uniform resize remains Document-owned | **Enforced today** | `resizeWhiteboardItems` transforms sketch points and affine curve bases in one transaction. Circular smart curves promote to `ellipse` / `elliptical-arc`; the Scene remains a projection. Rotated single rectangles carry a local orthonormal resize frame from the overlay gesture. |
...
| Arc endpoint closure gesture | `asy/engine/arc-closure.ts` — the only acquire/release policy. Creation, baked editing, and smart endpoint dragging use the same screen-space thresholds; local angular snap windows are a bug. |
...
| Pipeline B mechanics (translate / framed resize / rotate / smart-feature drag) | `whiteboard/smart-gestures.svelte.ts` (`SmartGestureController`: preview + commit solves; the `SmartGesture` arms of `ActiveGesture`) |
...
| Contextual constraint toolbar (placement, drag, relation/dimension actions) | `components/whiteboard/constraint-toolbar.svelte`, with its pure placement math in `constraint-toolbar.ts`. Placement is a two-stage pipeline: `auto` (projected selection geometry, board, and measured toolbar size only) → `position` (drag offset applied, clamped to the board). The first selected feature may supply a stable preferred side during additive selection. The drag writes the offset and may read `auto`; it must **never** read `position`, or the placement feeds back on itself. |
