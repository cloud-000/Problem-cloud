...
export interface OfflineMediaStore {
    ...
    stage(packageRevision: string, assets: OfflineAssetV1[]): Promise<StageAssetsResult>;
    ...
    return {
        async stage(packageRevision, assets) {
            ...
            for (const asset of assets) {
                ...
                try {
                    // `cors` (not `no-cors`): an opaque response has an unknown
                    // status and an unreadable size, so a 404 would be cached as
                    // though the image were present.
                    response = await fetch(url, { mode: "cors", credentials: "omit" });
                } catch (error) {
                    throw new OfflineAssetUnavailable(url, String(error));
                }
                if (!response.ok) {
                    throw new OfflineAssetUnavailable(url, `HTTP ${response.status}`);
                }
