...
--   4. Resolve per-problem response metadata at the content boundary: explicit
--      problem declaration, test default, then only source-structural fallbacks.
--      Ambiguous rows stay `unknown` / `needs_review`; runtime code never writes
--      inferred classifications back here.
--   5. Flag, never remove, live rows a scrape no longer covers (see
...
-- ---------------------------------------------------------------------------
-- Response-metadata resolution. These are content-import/backfill helpers, not
-- application RPCs. A problem declaration wins over its test default. Choice
-- shape is source structure, so it can safely classify legacy MCQ/short-answer
-- rows. Nothing infers proof from `not_applicable`: no choices means `unknown`
-- unless the scraper or test explicitly declares another response kind.
-- ---------------------------------------------------------------------------
create or replace function public.resolve_content_response_kind(
  problem_kind text,
  test_kind text,
  choices text[]
) returns text
...
as $$
  select coalesce(
    ...
    test_kind,
    case
      when coalesce(cardinality(choices), 0) > 1 then 'mcq'
      when cardinality(choices) = 1 then 'short_answer'
      else 'unknown'
    end
  );
$$;

create or replace function public.resolve_content_answer_status(
  problem_status text,
  test_status text,
  response_kind text,
  choices text[],
...
as $$
  select coalesce(
    ...
    case
      when answer_index >= 0
       and answer_index < coalesce(cardinality(choices), 0) then 'known'
      else null
    end,
    test_status,
    case response_kind
      when 'proof' then 'not_applicable'
      when 'mcq' then 'source_missing'
      when 'short_answer' then 'source_missing'
      else 'needs_review'
    end
  );
$$;

revoke execute on function public.resolve_content_response_kind(text, text, text[])
  from public, anon, authenticated;
revoke execute on function public.resolve_content_answer_status(text, text, text, text[], integer)
  from public, anon, authenticated;
grant execute on function public.resolve_content_response_kind(text, text, text[])
  to service_role;
grant execute on function public.resolve_content_answer_status(text, text, text, text[], integer)
  to service_role;


-- ---------------------------------------------------------------------------
-- Prepare a legacy DB for syncing. Idempotent; called at the top of
-- sync_scraped_content. Three jobs:
...
--   3. Backfill response metadata from explicit declarations and unambiguous
--      source structure. Rows without that evidence become unknown/needs_review.
-- ---------------------------------------------------------------------------
...
begin
  ...
  with resolved as (
    select
      p.id,
      t.answer_status as test_answer_status,
      public.resolve_content_response_kind(
        p.response_kind,
        t.response_kind,
        p.choices
      ) as response_kind
    ...
    where p.response_kind is null or p.answer_status is null
  )
  update public.problems p
  set
    response_kind = r.response_kind,
    answer_status = public.resolve_content_answer_status(
      p.answer_status,
      r.test_answer_status,
      r.response_kind,
      p.choices,
      p.answer_index
    )
...
begin
  ...
  -- problems: match on sync_key. Content columns are scraper-owned. Response
  -- metadata is resolved from a problem declaration, then its test default, then
  -- unambiguous source structure; difficulty/quality/verified/notes are
  -- seed-then-lock. test_id and n are
  ...
  with classified as (
    select ip.sync_key, t.id as test_id, ip.n, ip.aops_id, ip.statement,
           ...
           ip.answer_status as problem_answer_status,
           t.answer_status as test_answer_status,
  ...
  src as (
    select c.*,
           public.resolve_content_answer_status(
             c.problem_answer_status,
             c.test_answer_status,
             c.response_kind,
             c.choices,
             c.answer_index
           ) as answer_status
